...
import { Box, Container, Grid2, Typography, Button, Card, Chip, Input } from "@mui/material";
...
export default function CategoryCard({ categoryData, fetchCategories }) {
    ...
    const amountPerCategory = categoryData.sub_category.reduce((prev, curr) => prev + (section == 'Expense' ? parseFloat(curr.budget) : parseFloat(curr.expected_amount)), 0);
    ...
    const handleSubmitCategory = async (e) => {
        if (e.keyCode === 13) {
            const result = await updatedCategory(section, categoryData._id, {category: categoryInput});
            fetchCategories();
            setIsDoubleClicked(prev => !prev);
        }
    }
    ...
    return (
        ...
        <Grid2 size={4} minWidth='18rem'>
            <Card
                size="sm"
                variant="plain"
                sx={{ position: 'relative', textAlign: 'center', lineHeight: '2.5rem', minHeight: '2.5rem' }}
            ...
            <Box sx={{ bgcolor: 'white', borderRadius: '1.5rem', height: '22rem', marginTop: '1rem', padding: '1rem' }}>
                <Box sx={{ overflow: 'auto', height: '15rem', marginBottom: '0.5rem' }}>
                    <Chip
                        label={`Sub-category | ${ section == 'Expense' ? 'Budget' : 'Income' }`}
                    ...
                    {categoryData.sub_category.map(subCate => (
                        <BudgetBox key={subCate._id} categoryId={categoryData._id} subCategoryData={subCate} fetchCategories={fetchCategories} />
                    ))}
                </Box>
