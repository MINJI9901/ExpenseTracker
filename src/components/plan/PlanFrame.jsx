...
export default function PlanFrame({ sumOfAmount, setSumOfAmount }) {
    ...
    const fetchCategories = async () => {
        const response = await fetch(`http://localhost:3000/api/${section.toLowerCase()}/category?month=${selectedDate.getMonth()}&year=${selectedDate.getFullYear()}`)
    ...
    return (
        <Grid2
            container spacing={2}
            sx={{bgcolor: theme.palette.primary.main, width: '95%', height: 'fit-content', margin: 'auto', padding: '1rem'}}
        >
            ...
            >
                {categories.map(category => (
                    <CategoryCard key={category._id} categoryData={category} fetchCategories={fetchCategories}/>
                ))}
            </Grid2>
            <Grid2 size={{xs: 1.5, sm: 1, lg: 0.5}}>
                <Box
                    ...
                    variant="text"
                    sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', bgcolor: theme.palette.primary.dark, color: "black", borderRadius: '5px', width: '2.5rem', height: '25rem', writingMode: 'vertical-lr', textOrientation: 'upright', margin: 'auto', cursor: 'pointer' }}
                >
