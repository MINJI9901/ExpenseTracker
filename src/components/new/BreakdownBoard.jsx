...
export default function BreakdownBoard({ getBreakdownData, breakdownData, newBreakdownData }) {
    ...
    return (
        <Box>
            ...
            <Box sx={{bgcolor: theme.palette.primary.main, height: '30rem', padding: '0.5rem', my: '1rem'}}>
                <Box sx={{display: 'flex', justifyContent: 'space-evenly', textAlign: 'center', borderRadius: '5px', color: 'gray',  padding: '0.2rem'}}>
                    <Typography sx={{fontSize: 'smaller', width: '10%', overflowX: 'auto'}}>Date</Typography>
                    <Typography sx={{fontSize: 'smaller', width: '20%', overflowX: 'auto'}}>Name</Typography>
                    <Typography sx={{fontSize: 'smaller', width: '10%', overflowX: 'auto'}}>Amount</Typography>
                    <Typography sx={{fontSize: 'smaller', width: '20%', overflowX: 'auto'}}>Category</Typography>
                    <Typography sx={{fontSize: 'smaller', width: '20%', overflowX: 'auto'}}>Sub-category</Typography>
                    <Typography sx={{fontSize: 'smaller', width: '10%', overflowX: 'auto'}}>Modify</Typography>
                </Box>
                <Box sx={{overflowY: 'auto', height: '90%'}}>
                    {breakdownData.map(breakdown => (
                        newBreakdownData && newBreakdownData._id === breakdown._id
                        ? <Breakdown key={breakdown._id} breakdown={breakdown} getBreakdownData={getBreakdownData} isNew={true}></Breakdown>
                        : <Breakdown key={breakdown._id} breakdown={breakdown} getBreakdownData={getBreakdownData}></Breakdown>
                    ))}
                </Box>
            </Box>
        </Box>
