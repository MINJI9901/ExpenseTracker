...
export async function getBreakdown(section, date) {
    const response = await fetch(`http://localhost:3000/api/${section.toLowerCase()}?start=${date.start}&end=${date.end}`)
...
export async function addBreakdown(section, body) {
    const response = await fetch(`http://localhost:3000/api/${section.toLowerCase()}`, {
        method: 'POST',
