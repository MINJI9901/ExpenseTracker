...
export async function PUT(req) {
    ...
    if (content.category) {
        updatedCategory = await IncomeCategory.findByIdAndUpdate(id,
            { $set: content },
            { new: true }
        );
    } else {
...
export async function DELETE(req) {
    ...
    if (typeof id !== 'object') {
        ...
        const deletedIncome = await Income.deleteMany({category: deletedCategory});
        ...
        const category = await IncomeCategory.findByIdAndUpdate(categoryId, 
            {$pull: { sub_category: {_id: subCategoryId} }},
            { new: true }
        );

        const deletedIncome = await Income.deleteMany({'sub_category._id': subCategoryId})
