...
export async function PUT(req) {
    ...
    if (content.category) {
        updatedCategory = await ExpenseCategory.findByIdAndUpdate(id,
            { $set: content },
            { new: true }
        );
    } else {
...
export async function DELETE(req) {
    ...
    if (typeof id !== 'object') {
        ...
        const deletedExpenses = await Expense.deleteMany({category: deletedCategory});
        ...
        const category = await ExpenseCategory.findByIdAndUpdate(categoryId, 
            {$pull: { sub_category: {_id: subCategoryId} }},
            { new: true }
        );

        const deletedExpenses = await Expense.deleteMany({'sub_category._id': subCategoryId})
